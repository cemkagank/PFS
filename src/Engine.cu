...
Engine::Engine() {
  ...
  mat.maps[MATERIAL_MAP_DIFFUSE].color =
      Color{(unsigned char)(particle_color[0] * 255),
...
void Engine::init_cuda() {
  ...
  cudaMemcpy(d_positions, h_postions, simulation_size * sizeof(float3),
...
void Engine::Update() {
  ...
  cudaMemcpy(h_positions, d_postions, simulation_size * sizeof(float3),
  ...
  for (int i = 0; i < simulation_size; i++) {
    transforms[i] =
        MatrixTranslate(h_positions[i].x, h_positions[i].y, h_positions[i].z);
  }
}

void Engine::Reset() { simulation_size = 0; }
